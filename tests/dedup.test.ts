...
const newCand: DistillCandidate = {
  ...
  scopeType: 'project', runtime: null, distillAction: 'new', origin: 'agent-observed', evidence: null, subjectSlug: null,
}
...
test('judgeDuplicates merges same-batch sibling (new-j duplicateOf)', async () => {
  const a: DistillCandidate = { title: '[category:invariant] 退款14天', bodyMd: 'b', scopeType: 'project', runtime: null, distillAction: 'new', origin: 'agent-observed', evidence: null, subjectSlug: null }
  const b: DistillCandidate = { title: '[category:invariant] 退款须在14天内', bodyMd: 'b', scopeType: 'project', runtime: null, distillAction: 'new', origin: 'agent-observed', evidence: null, subjectSlug: null }
  const v = await judgeDuplicates({
...
test('judgeDuplicates merges same-rule-different-facet siblings', async () => {
  ...
  // 应让 LLM 判为重复，只留第一条。
  const a: DistillCandidate = { title: '[category:invariant] 退款须在发货后14天内', bodyMd: '14天退款窗口', scopeType: 'project', runtime: null, distillAction: 'new', origin: 'agent-observed', evidence: null, subjectSlug: null }
  const b: DistillCandidate = { title: '[category:invariant] 退款规则的14天期限不可被丢弃', bodyMd: '退款期限是14天', scopeType: 'project', runtime: null, distillAction: 'new', origin: 'agent-observed', evidence: null, subjectSlug: null }
  const v = await judgeDuplicates({
    newCandidates: [a, b], existing: [],
    callLLM: async () => JSON.stringify({ verdicts: [{ index: 0, isDuplicate: false }, { index: 1, isDuplicate: true, duplicateOfId: 'new-0' }] }),
  })
