...
> 判定原则：当你犹豫"这个改动要不要先 brainstorming"时，默认答案是"要"。常规改动也先想清楚再动手。

### 2. 落档后清理 `.superpowers/sdd`（强制）

spec、plan 两份文件落档到 `docs/superpowers/specs/` 与 `docs/superpowers/plans/` 之后、**开始写任何代码之前**，必须删除 `.superpowers/sdd/` 目录下的**所有文件**——这是 brainstorming 流程的中间工作产物，残留会污染下一次设计的上下文。

- **时机**：spec、plan 已写入 `docs/superpowers/` 即触发；切分支、写代码之前完成。
- **不得跳过**：这一步是"设计阶段结束 → 执行阶段开始"的闸门，未清理不算落档完成，不得直接进入分支 / 编码。
...
### 「e2e 测试」= 5 个 e2e/live 测试文件全集

用户说「跑 e2e 测试」指跑**这 5 个**（不是只跑 `npm run test:live` 的 3 个 live，也不是只跑 mock 的 2 个）。2 个 mock + 3 个真打 LLM，一次跑全：

```bash
MEMSIDE_RUN_LIVE=1 bun test tests/e2e.test.ts tests/e2e-distill-batching.test.ts tests/live-distill.test.ts tests/live-judge.test.ts tests/live-llm-failure-resume.test.ts
```

| 文件 | 类型 | 触发方式 |
|---|---|---|
| `tests/e2e.test.ts` | mock LLM | 默认 `bun test` 即跑（验数据流契约） |
| `tests/e2e-distill-batching.test.ts` | mock LLM | 默认 `bun test` 即跑 |
| `tests/live-distill.test.ts` | 真打 LLM | 需 `MEMSIDE_RUN_LIVE=1` + 凭证 |
...
3 个 live 文件受双守卫（`loadClaudeCreds` 返回非空 apiKey **且** `MEMSIDE_RUN_LIVE=1`）保护，默认 `bun test` 与 CI 全 skip 不真打模型。`npm run test:live`（= `cross-env MEMSIDE_RUN_LIVE=1 bun test tests/live-*.test.ts`）跑全部 3 个 live 文件，但**不含** 2 个 mock e2e——要跑全 5 个用上面的全集命令。真打模型的 live 测试 distill 流式约 170-210s，整套约 2 分钟，给足 timeout。
