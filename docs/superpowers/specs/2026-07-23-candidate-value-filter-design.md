...
| 9 | judgeValue JSON 解析 | 镜像 dedup：`callWithRetry` + `extractJsonObject` + `valueShouldRetry` 校验重试 + 保守 fallback | 一致性；PR #7 已合入 `extractJsonObject`/`callWithRetry`，valueFilter 直接复用（commit 0b4961f） |
...
- 内部走 `callWithRetry({call, system, user, shouldRetry: valueShouldRetry(n)})`（`./retry`，同 dedup）：`extractJsonObject` 容忍 markdown 围栏包裹的 JSON；`valueShouldRetry` 校验 `verdicts` 数组与每条 `index`（数值、在 `[0,n)` 内）/`category`（6 类之一）合法性，不合法则带反馈重试（默认 2 次）。
...
- index 缺失 / 越界 / category 不在 6 个里 / LLM 抛错 / 非 JSON（重试耗尽）/ 缺 `verdicts` -> `{keep:true, valueClass:null}`（`callWithRetry` 重试耗尽或外层 `catch` 兜底均走此分支）
