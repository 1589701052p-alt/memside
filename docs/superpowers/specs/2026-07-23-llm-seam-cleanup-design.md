...
- **`tests/llm.test.ts`**（由 `tests/anthropic.test.ts` 改名；`mock.module('@anthropic-ai/sdk', ...)` 不变）：
...
- **`tests/distiller.test.ts` / `tests/dedup.test.ts` / `tests/scheduler.test.ts` / `tests/daemon.test.ts` / `tests/e2e.test.ts`**：mock 字段 `callAnthropic:` -> `callLLM:`（~30 处，纯机械；这些 mock 忽略入参，可选第 3 参 `opts` 不影响）。逻辑不变，靠现有用例保持绿验证重构无行为回归。
...
- 新增：`src/llm.ts`
- 改名：`tests/anthropic.test.ts` -> `tests/llm.test.ts`
- 改：`src/anthropic.ts`（`makeLLMCall` + opts + import `./llm`）、`src/memory/distiller.ts`、`src/memory/dedup.ts`、`src/scheduler.ts`、`src/daemon.ts`
